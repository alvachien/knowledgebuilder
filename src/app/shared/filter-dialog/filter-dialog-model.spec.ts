...
describe('filter-dialog-model', () => {
  ...
  describe('seedTree', () => {
    it('seeds an empty/undefined definition into an empty AND root', () => {
      ...
      expect(explicit.join).toBe(FilterJoinType.OR);
      expect(explicit.members).toEqual([]);
    });

    it('seeds flat conditions into leaves carrying property, operator and value', () => {
      const root = seedTree(
        {
      ...
      const [word, num] = root.members as SharedFilterDialogLeaf[];
    ...
    it('preserves nesting to any depth', () => {
      ...
      const orGroup = root.members[1] as SharedFilterDialogNode;
    ...
    it('folds an enum OR-of-Equal group into one multi-choice leaf, and seeds bare Equals', () => {
      ...
      const folded = root.members[0] as SharedFilterDialogLeaf;
  ...
  describe('validateTree', () => {
    ...
    it('nested groups need two or more members; the root is exempt', () => {
      ...
      // root with fewer than two members is fine…
      expect(validateTree(rootOf(wordLeaf('a')), schema).canSubmit).toBe(true);
      expect(validateTree(rootOf(), schema).canSubmit).toBe(true);
      // …but a deep nested violation is still caught.
  ...
  describe('describeMember', () => {
    it('labels nodes with their join and leaves with their condition phrase', () => {
      ...
      expect(describeMember(root.members[0], schema, labels)).toBe('word contains app');
      expect(describeMember(root.members[1], schema, labels)).toBe('group (OR)');
    });
