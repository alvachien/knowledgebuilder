...
export class ODataService {
  ...
  public getUserDetail(): Observable<InvitedUser> {
    return this.authService.isAuthenticated().pipe(mergeMap(val => {
      if (val && this.currentUserDetail !== null) {
        return of(this.currentUserDetail);
      } else if (!val) {
        return throwError(() => new Error(this.expertModeFailMsg));
      }

      return this.authService.getAccessToken().pipe(mergeMap(val2 => {
        ...
        headers = headers.append(this.contentType, this.appJson)
          ...
          .append('Authorization', 'Bearer ' + val2);;
  ...
  public getUserCollections(top = 30, skip = 0, sort?: string, filter?: string): Observable<{ totalCount: number; items: UserCollection[] }> {
    return this.authService.isAuthenticated().pipe(mergeMap(islogin => {
      ...
      return this.authService.getUserData().pipe(mergeMap(ud => {
        let headers: HttpHeaders = new HttpHeaders();
        headers = headers.append(this.contentType, this.appJson)
          .append(this.strAccept, this.appJson);
        ...
        if (filter) {
          params = params.append('$filter', `${filter} and User eq '${ud.sub}'`);
        } else {
          params = params.append('$filter', `User eq '${ud.sub}'`);
        }
