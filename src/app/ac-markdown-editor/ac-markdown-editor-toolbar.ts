...
export class ACMEditorToolbarDevtools extends ACMEditorToolbarItem {
  constructor(editor: IACMEditor, menuItem: IACMEToolbarItem) {
    ...
    this.element.children[0].innerHTML = menuItem.icon
      // || bugSVG;
      || `<svg version='1.1' xmlns='http://www.w3.org/2000/svg' width='32' height='32' viewBox='0 0 32 32'>
    <path d='M32 18.24c0 0.7-0.58 1.28-1.28 1.28h-4.48c0 2.5-0.54 4.38-1.34 5.8l4.16 4.18c0.5 0.5 0.5 1.3 0 1.8-0.24 0.26-0.58 0.38-0.9 0.38s-0.66-0.12-0.9-0.38l-3.96-3.94s-2.62 2.4-6.020 2.4v-17.92h-2.56v17.92c-3.62 0-6.26-2.64-6.26-2.64l-3.66 4.14c-0.26 0.28-0.6 0.42-0.96 0.42-0.3 0-0.6-0.1-0.86-0.32-0.52-0.48-0.56-1.28-0.1-1.82l4.040-4.54c-0.7-1.38-1.16-3.16-1.16-5.48h-4.48c-0.7 0-1.28-0.58-1.28-1.28s0.58-1.28 1.28-1.28h4.48v-5.88l-3.46-3.46c-0.5-0.5-0.5-1.3 0-1.8s1.3-0.5 1.8 0l3.46 3.46h16.88l3.46-3.46c0.5-0.5 1.3-0.5 1.8 0s0.5 1.3 0 1.8l-3.46 3.46v5.88h4.48c0.7 0 1.28 0.58 1.28 1.28zM22.4 6.72h-12.8c0-3.54 2.86-6.4 6.4-6.4s6.4 2.86 6.4 6.4z'></path>
    </svg>`;

    this.element.addEventListener(getEventName(), async () => {
      if (this.element.children[0].className.indexOf(`${classPrefix}-menu--current`) > -1) {
        this.element.children[0].className =
          this.element.children[0].className.replace(` ${classPrefix}-menu--current`, '');
        editor.devtools.element.style.display = 'none';
        if (editor.wysiwyg) {
          const padding =
            (editor.wysiwyg.element.parentElement.scrollWidth - editor.options.preview.maxWidth) / 2;
          editor.wysiwyg.element.style.paddingLeft = `${Math.max(10, padding)}px`;
          editor.wysiwyg.element.style.paddingRight = `${Math.max(10, padding)}px`;
        }
      } else {
        this.element.children[0].className += ` ${classPrefix}-menu--current`;
        editor.devtools.element.style.display = 'block';
        if (editor.wysiwyg) {
          const padding =
            ((editor.wysiwyg.element.parentElement.scrollWidth / 2) - editor.options.preview.maxWidth) / 2;
          editor.wysiwyg.element.style.paddingLeft = `${Math.max(10, padding)}px`;
          editor.wysiwyg.element.style.paddingRight = `${Math.max(10, padding)}px`;
        }
...
export class ACMEditorToolbarPreview extends ACMEditorToolbarItem {
  constructor(editor: IACMEditor, menuItem: IACMEToolbarItem) {
    ...
    if (editor.currentPreviewMode === 'preview') {
      this.element.children[0].className =
        `${classPrefix}-tooltipped ${classPrefix}-tooltipped__${menuItem.tipPosition} ${classPrefix}-menu--current`;
    }
  ...
  public _bindEvent(editor: IACMEditor) {
    this.element.children[0].addEventListener(getEventName(), function() {
      if (editor.currentPreviewMode === 'preview') {
        ...
        editor.editor.element.style.display = 'none';
        editor.currentPreviewMode = 'preview';
      }
...
export class ACMEditorToolbarWYSIWYG extends ACMEditorToolbarItem {
  constructor(editor: IACMEditor, menuItem: IACMEToolbarItem) {
    super(editor, menuItem);
  ...
  public _bindEvent(editor: IACMEditor) {
    this.element.children[0].addEventListener(getEventName(), function(event) {
      if (this.className.indexOf(`${classPrefix}-menu--current`) > -1) {
        this.className = this.className.replace(` ${classPrefix}-menu--current`, '');
        editor.wysiwyg.element.style.display = 'none';
        if (editor.currentPreviewMode === 'both') {
          editor.editor.element.style.display = 'block';
          editor.preview.element.style.display = 'block';
        } else if (editor.currentPreviewMode === 'preview') {
          editor.preview.element.style.display = 'block';
        } else if (editor.currentPreviewMode === 'editor') {
          editor.editor.element.style.display = 'block';
        }
        if (editor.toolbar.elements.format) {
          editor.toolbar.elements.format.style.display = 'block';
        }
        if (editor.toolbar.elements.both) {
          editor.toolbar.elements.both.style.display = 'block';
        }
        if (editor.toolbar.elements.preview) {
          editor.toolbar.elements.preview.style.display = 'block';
        }

        const wysiwygHTML = editor.lute.EditorDOMMarkdown(editor.wysiwyg.element.innerHTML);
        formatRender(editor, wysiwygHTML[0] || wysiwygHTML[1], undefined, false);
        ...
        editor.editor.element.style.display = 'none';
        editor.preview.element.style.display = 'none';
        editor.wysiwyg.element.style.display = 'block';

        if (editor.toolbar.elements.format) {
          editor.toolbar.elements.format.style.display = 'none';
        }
        if (editor.toolbar.elements.both) {
          editor.toolbar.elements.both.style.display = 'none';
        }
        if (editor.toolbar.elements.preview) {
          editor.toolbar.elements.preview.style.display = 'none';
        }
        renderDomByMd(editor, getText(editor.editor.element));
      ...
      if (editor.toolbar.elements.headings) {
        (editor.toolbar.elements.headings.children[1] as HTMLElement).style.display = 'none';
      }
      if (editor.toolbar.elements.emoji) {
        (editor.toolbar.elements.emoji.children[1] as HTMLElement).style.display = 'none';
      }
      if (editor.devtools && editor.devtools.ASTChart && editor.devtools.element.style.display === 'block') {
        editor.devtools.ASTChart.resize();
      }
    ...
    options.toolbar.forEach((menuItem: IACMEToolbarItem, i: number) => {
      ...
      switch (menuItem.name) {
        ...
        case 'format':
          ...
          break;
        case 'wysiwyg':
          if (editor.options.mode.indexOf('only') === -1) {
            menuItemObj = new ACMEditorToolbarWYSIWYG(editor, menuItem);
          }
          break;
        case 'devtools':
          menuItemObj = new ACMEditorToolbarDevtools(editor, menuItem);
          break;
        default:
