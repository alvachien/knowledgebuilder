 ...
 * (there is no by-word selection for Chinese items). `OK` returns a
 * `VocabularySelectOption`; Cancel / backdrop / Esc return `undefined` so the
 * caller leaves the selection untouched.
 */
@Component({
  selector: 'app-chinese-exercises-select-dlg',
  templateUrl: 'chinese-exercises-select-dialog.html',
  imports: [
    MatFormFieldModule,
    FormsModule,
    MatInputModule,
...
export class ChineseExercisesSelectDialogComponent {
  readonly dialogRef = inject(MatDialogRef<ChineseExercisesSelectDialogComponent>);
