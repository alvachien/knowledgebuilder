...
describe('TranslateReviewSessionStore', () => {
  ...
  let mockAIService: {
    getTTS: ReturnType<typeof vi.fn>;
  };
  ...
  const item = (ensent: string, cnsent = '测试句子', itemId?: number): SentenceReviewQueueItem => ({
    ensent,
    cnsent,
    rating: 0,
    itemId,
  ...
  beforeEach(() => {
    ...
    mockAIService = {
      getTTS: vi.fn().mockReturnValue(of({ audioFileUrl: 'audio/tts.mp3' })),
    };
    mockRatingService = {
  ...
  describe('start', () => {
    it('returns false and stays reset for an empty queue', () => {
      ...
      expect(mockAIService.getTTS).not.toHaveBeenCalled();
      expect(mockRatingService.getRatings).not.toHaveBeenCalled();
    });

    it('builds the queue, resets the cursor and speaks the first sentence via TTS', () => {
      expect(store.start([item('Hello world'), item('Good morning')], false, 0)).toBe(true);

      expect(store.queue().length).toBe(2);
      expect(store.cursor()).toBe(0);
      expect(store.progress()).toBe(50);
      expect(mockAIService.getTTS).toHaveBeenCalledWith('Hello world');
      expect(mockAudioService.playSound).toHaveBeenCalledWith('audio/tts.mp3', false);
    });

    it('does not speak when voice is disabled', () => {
      store.start([item('Hello world')], true, 0);
    ...
    it('pre-populates ratings from the API into the queue', () => {
      ...
      store.start(
        [item('Hello world', '你好世界', 10), item('Good morning', '早上好', 20), item('Thank you', '谢谢', 30)],
        false,
        1
      );
    ...
    it('does not load ratings when contentId is 0', () => {
      store.start([item('Hello world', '你好世界', 10)], false, 0);

      expect(mockRatingService.getRatings).not.toHaveBeenCalled();
    });
  });

  describe('ratings preload vs in-session ratings', () => {
    it('a late preload must not overwrite a rating the user already changed', () => {
      ...
      mockRatingService.upsertRating.mockReturnValue(of({ contentId: 1, itemId: 10, rating: 5 }));
      store.start([item('Hello world', '你好世界', 10)], true, 1);

      // The user rates the sentence before the (slow) preload response lands
      // and the save is confirmed right away.
      store.rateCurrent(5);
    ...
    it('a late preload still populates items the user has not rated', () => {
      ...
      mockRatingService.getRatings.mockReturnValue(preload$.asObservable());
      mockRatingService.upsertRating.mockReturnValue(of({ contentId: 1, itemId: 10, rating: 5 }));
      store.start([item('Hello world', '你好世界', 10), item('Good morning', '早上好', 20)], true, 1);
  ...
  describe('navigation', () => {
    beforeEach(() => {
      store.start([item('a'), item('b'), item('c')], true, 0);
    });

    it('moves to the previous sentence and updates progress', () => {
      store.next();
    ...
    it('does not move before the first sentence', () => {
      store.previous();

      expect(store.cursor()).toBe(0);
    });

    it('does not move past the last sentence', () => {
      store.next();
      store.next();
      store.next();
    ...
    it('speaks the sentence on navigation when voice is enabled', () => {
      store.start([item('Hello world'), item('Good morning')], false, 0);
      mockAIService.getTTS.mockClear();

      store.next();

      expect(mockAIService.getTTS).toHaveBeenCalledWith('Good morning');
    });
  });

  describe('voice (TTS generation guard)', () => {
    it('ignores a late TTS response for a sentence the cursor has already left', () => {
      const first$ = new Subject<{ audioFileUrl: string }>();
      mockAIService.getTTS.mockReturnValueOnce(first$.asObservable());

      store.start([item('Hello world'), item('Good morning')], false, 0);
      store.next(); // 'Hello world' is superseded by 'Good morning's own fetch

      first$.next({ audioFileUrl: 'audio/stale.mp3' }); // too late

      expect(mockAudioService.playSound).not.toHaveBeenCalledWith('audio/stale.mp3');
    });

    it('drops the playback once the session is quit/reset', () => {
      const tts$ = new Subject<{ audioFileUrl: string }>();
      mockAIService.getTTS.mockReturnValueOnce(tts$.asObservable());

      store.start([item('Hello world')], false, 0);
      store.quit();

      tts$.next({ audioFileUrl: 'audio/stale.mp3' });

      expect(mockAudioService.playSound).not.toHaveBeenCalled();
    });

    it('survives a TTS error', () => {
      mockAIService.getTTS.mockReturnValueOnce(throwError(() => new Error('TTS failed')));
  ...
  describe('rateCurrent', () => {
    ...
    it('does not save when contentId is 0', () => {
      store.start([item('Hello world', '你好世界', 10)], true, 0);
    ...
    it('does not save when itemId is undefined', () => {
      store.start([item('Hello world', '你好世界', undefined)], true, 1);
  ...
  describe('auto mode', () => {
    ...
    it('does not double-start when already running', () => {
      vi.useFakeTimers();
      store.start([item('a'), item('b')], true, 0);
    ...
    it('auto-advances to the next sentence every N seconds', () => {
      vi.useFakeTimers();
      store.start([item('a'), item('b'), item('c')], true, 0);
    ...
    it('restarts from the first sentence regardless of the current cursor', () => {
      vi.useFakeTimers();
      store.start([item('a'), item('b'), item('c'), item('d')], true, 0);
      store.next();
      store.next(); // cursor at 2, mid-queue

      store.enableAutoMode();

      // Always jumps to the first sentence, even though the cursor was mid-queue.
      expect(store.isAutoMode()).toBe(true);
      expect(store.cursor()).toBe(0);
      expect(store.progress()).toBe(25);

      // Auto-advances from the first sentence through the queue.
      vi.advanceTimersByTime(5000);
      expect(store.cursor()).toBe(1);
      vi.advanceTimersByTime(5000);
      expect(store.cursor()).toBe(2);
      ...
      // Reaches the last sentence and self-stops on the next tick.
      vi.advanceTimersByTime(5000);
      expect(store.isAutoMode()).toBe(false);
      expect(store.cursor()).toBe(3);
    ...
    it('speaks the first sentence when auto mode starts (voice enabled)', () => {
      store.start([item('Hello world'), item('Good morning')], false, 0);
      mockAIService.getTTS.mockClear();
  ...
  describe('quit', () => {
    ...
    it('includes preloaded ratings (unchanged sentences keep their stored rating)', () => {
      mockRatingService.getRatings.mockReturnValue(
        of([{ contentId: 1, itemId: 10, rating: 3 }])
      );
      store.start([item('Hello world', '你好世界', 10)], true, 1);
  ...
  describe('reset', () => {
    it('clears every piece of session state', () => {
      store.start([item('Hello world', '你好世界', 10)], false, 1);
      store.rateCurrent(3);
    ...
    it('cancels an in-flight ratings load: a late response must not leak into the next session', () => {
      ...
      store.reset();
      // New session over another file; item ids are per-file, so itemId 10
      // collides - the stale response must not pre-populate it.
      mockRatingService.getRatings.mockReturnValueOnce(of([]));
      store.start([item('Good morning', '早上好', 10)], true, 2);
  ...
  describe('destruction', () => {
    it('stops the auto timer when the store is destroyed (leaving the page mid-auto-mode)', () => {
      ...
      // auto-play timer - otherwise it keeps advancing and speaking sentences.
    ...
    it('cancels in-flight rating saves on destroy', () => {
    ...
    it('cancels an in-flight ratings load on destroy so a late response cannot leak', () => {
      const late$ = new Subject<UserLearningRating[]>();
      mockRatingService.getRatings.mockReturnValueOnce(late$.asObservable());
      store.start([item('Hello world', '你好世界', 10)], true, 1);
