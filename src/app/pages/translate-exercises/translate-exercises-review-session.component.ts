...
/**
 * Review screen of the sentence (translation) exercises page: progress bar,
 * auto-play, previous/next/quit controls and the current sentence pair (EN +
 * CN) with its rating toggles. All review state lives in the injected session
 * store; this component renders it and owns the keyboard listener, so
 * keystroke handling is registered only while the review screen is alive (the
 * container's @switch destroys it).
 */
@Component({
  selector: 'app-translate-exercises-review-session',
  ...
  templateUrl: './translate-exercises-review-session.component.html',
  styleUrl: './translate-exercises-review-session.component.scss',
})
export class TranslateExercisesReviewSessionComponent {
  /** Interval choices offered by the seconds-per-sentence selector. */
  protected readonly autoModeSecondChoices = [3, 5, 10];

  protected readonly store = inject(TranslateReviewSessionStore);

  readonly quit = output<void>();

  /**
   * Review keyboard shortcuts: 1-5 rate the current sentence, ArrowUp/Down
   * nudge the rating, ArrowLeft/Right move through the queue, Esc quits.
   * Manual prev/next are ignored while auto mode is running.
   */
  @HostListener('document:keyup', ['$event'])
  handleKeyboardEvent(event: KeyboardEvent) {
    // Browser/app chords are never review input: Alt+ArrowLeft/Right belongs
    // to browser history (the sentence must not also change), Ctrl/Alt+1…5
    // must not rate the sentence.
    if (event.ctrlKey || event.altKey || event.metaKey) {
      return;
    }

    // Keys typed into a form control belong to that control: arrows navigate
    // the seconds-per-sentence select's options and Escape closes its panel -
    // neither must re-rate the current sentence nor quit the session. The open
    ...
    // — the call here is kept for parity / defensive coverage.
   ...
   * ArrowUp/Down/Left/Right, which the keyup preventDefault cannot.
   */
